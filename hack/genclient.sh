...
"$BASEDIR"/bin/client-gen \
  ...
  --input-base "" \
  --input github.com/sap/cert-manager-cop/tmp/gen/apis/operator.kyma-project.io/v1alpha1 \
  --go-header-file "$BASEDIR"/hack/boilerplate.go.txt \
  --output-package github.com/sap/cert-manager-cop/pkg/client/clientset \
  --output-base "$TEMPDIR"/pkg/client \
  --plural-exceptions CertManager:certmanagers

"$BASEDIR"/bin/lister-gen \
  --input-dirs github.com/sap/cert-manager-cop/tmp/gen/apis/operator.kyma-project.io/v1alpha1 \
  --go-header-file "$BASEDIR"/hack/boilerplate.go.txt \
  --output-package github.com/sap/cert-manager-cop/pkg/client/listers \
  --output-base "$TEMPDIR"/pkg/client \
  --plural-exceptions CertManager:certmanagers
...
"$BASEDIR"/bin/informer-gen \
  ...
  --go-header-file "$BASEDIR"/hack/boilerplate.go.txt \
  --output-package github.com/sap/cert-manager-cop/pkg/client/informers \
  --output-base "$TEMPDIR"/pkg/client \
  --plural-exceptions CertManager:certmanagers
...
mv "$TEMPDIR"/pkg/client/github.com/sap/cert-manager-cop/pkg/client "$BASEDIR"/pkg
